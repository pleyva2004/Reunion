...
- `common_free` is computed over `resolved` participants only. **v1:** unresolved
  participants are excluded from the intersection but do **not** block the trip;
  RocketRide forwards `connect_url` and may still propose dates. See
  `docs/contracts/roster-to-calendar-integration.md`.
